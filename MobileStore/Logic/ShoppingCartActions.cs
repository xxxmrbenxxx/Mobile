...
namespace MobileStore.Logic
{
    public class ShoppingCartActions : IDisposable
    {
        public string ShoppingCartId { get; set; }
        private MobileContext _db = new MobileContext();
        ...
        public void AddToCart(int id)
        {
            ...
            if (cartItem == null)
            {
                ...
                cartItem = new CartItem
                {
                    ...
                    Mobile = _db.Mobiles.SingleOrDefault(
               p => p.MoblieID == id),
                    Quantity = 1,
       ...
       CartItemUpdates)
        {
            using (var db = new MobileStore.Models.MobileContext())
            {
                try
                {
                    ...
                    foreach (var cartItem in myCart)
                    {
                        ...
                        {
                            if (cartItem.Mobile.MoblieID == CartItemUpdates[i].BookId)
                            {
                                if (CartItemUpdates[i].PurchaseQuantity < 1 ||
                               CartItemUpdates[i].RemoveItem == true)
                                {
                                    RemoveItem(cartId, cartItem.MobileId);
                                }
                                else
                                {
                                    UpdateItem(cartId, cartItem.MobileId,
        ...
        {
            using (var _db = new MobileStore.Models.MobileContext())
            {
                try
                {
                    var myItem = (from c in _db.ShoppingCartItems
                                  where c.CartId == removeCartID && c.Mobile.MoblieID ==
                                 removeBookID
        ...
        quantity)
        {
            using (var _db = new MobileStore.Models.MobileContext())
            {
                ...
                {
                    var myItem = (from c in _db.ShoppingCartItems
                                  where c.CartId == updateCartID && c.Mobile.MoblieID ==
                                 updateBookID
                                  select c).FirstOrDefault();
                    if (myItem != null)
                    {
